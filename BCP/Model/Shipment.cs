...
namespace BCP.Model
{
    class Shipment
    {
        ...
        public static bool deleteLoadCreateby(string Shipmentnumber, string createby)
        {

            if (Utils.IsValidnumber(Shipmentnumber))
            {
                ...
                string StringQuery = "Delete from tbl_shipment_list where tbl_shipment_list.shipment = @shipment and tbl_shipment_list.Createby = @Createby";
                conn.Open();


                OleDbCommand comm = new OleDbCommand(StringQuery, conn);

                comm.Parameters.AddWithValue("@shipment", Shipmentnumber);
        ...
        public static bool makeLoadTemp(string username)
        {
            // buoc 1 insert order tem với 


            bool dekq = Model.Shipment.deleteLoadCreateby("0", username);
            //  string st = "Delete from tbl_list_Order where tbl_list_Order.Document = @Ordernumber";
            string connection_string = Utils.getAccessConnectionstring();
            OleDbConnection conn = new OleDbConnection(connection_string);
            ...
            conn.Open();
            string StringQuery = @"INSERT INTO tbl_shipment_list( shipment,   Createby ) 
                                    VALUES ( @shipment, @Created_by )";
            OleDbCommand comm = new OleDbCommand(StringQuery, conn);
        ...
        public static string getShipmentnumbertemp()
        {
            string Shipmentnumbertemp = "";
            string username = Utils.getUsername();
            Model.Shipment.makeLoadTemp(username);
            ...
            string StringQuery = @"Select tbl_shipment_list.ID from  tbl_shipment_list where tbl_shipment_list.shipment = @shipment and  tbl_shipment_list.Createby = @Createby ";//, @Customer_Code )"; //'%test%'
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                Shipmentnumbertemp = dr["ID"].ToString();
            }
            ...
            return Shipmentnumbertemp;



        }

        public static string getShipmentofOrders(string OrderNumber)
        {
            string shipmentget = "";
            string connection_string = Utils.getAccessConnectionstring();
            ...
            string StringQuery = @"Select tbl_list_Order.Shipment from  tbl_list_Order where tbl_list_Order.Document = @Document 
                                    and tbl_list_Order.Shipment <> @Shipment";//, @Customer_Code )"; //'%test%'
            ...
            comm.Parameters.AddWithValue("@Document", OrderNumber);
            comm.Parameters.AddWithValue("@Shipment", 0);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                   shipmentget = dr["Shipment"].ToString();
            }
